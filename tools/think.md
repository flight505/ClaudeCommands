...
## ML/Bioinformatics Applications

### Example: Model Architecture Decision

```
/think comparing transformer vs LSTM architectures for genomic sequence analysis - what are the tradeoffs for processing long sequences?
```

### Example: Research Methodology

```
/think best approach for designing experiments to validate a new bioinformatics algorithm - what validation strategies should we use?
```

### Example: Technical Problem Solving

```
/think why is model accuracy plateauing at 85% on validation set - what could be the root causes?
```
